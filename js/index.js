...
function stand() {
    ...
    if(isSplitMode) { // スプリットモード時の処理
        ...
        setTimeout(() => {    
            ...
            // 勝者の判定
            let win = fight1 ? 'Player' : 'Dealer';
            resultdisplay(win);
            
            calculateWinnings(fight1, blackjack1, double1, fight2, blackjack2, double2, insuranceAvailable, Bet1, Bet2)
            gameOver = true;
...
function determineWinner(fight1, fight2) {
    if (fight1 && !fight2) return 'Player Hand 1';
    if (!fight1 && fight2) return 'Player Hand 2';
    if (fight1 && fight2) return 'Both Hands';
    return 'Dealer';
}
...
function determineFight(dealer, player) {
    ...
    letjudge = player > 21 ? false : true;
    judge = dealer > 21 ? true: player > dealer ? true : false;
    return judge;
}
// セット処理
function reset() {
